...
- Web Skia setup: the custom `index.web.tsx` entry waits for CanvasKit/WASM via `LoadSkiaWeb()` before registering Expo Router; `postinstall` refreshes `public/canvaskit.wasm` after Skia upgrades. App canvases go through `components/ui/webSafeCanvas.tsx` on web so each draw releases its WebGL context and an unavailable surface does not crash the router with CanvasKit's `rangeMin` error; native keeps Skia's regular renderer.
...
The `(tabs)` stack screen keeps the navigator's area behind the transparent bar on Havit's `ink` surface, and the root navigation theme makes its card/background `ink` with a transparent border. Web and Android additionally use an absolute transparent `tabBarStyle`, so their scenes continue behind the floating bar instead of ending at a black reserved strip. iOS intentionally keeps the default container because styling `tabBarStyle` there was proven to disable the native touch surface under Fabric. The bar itself remains implemented through `tabBarBackground` and per-item buttons, preserving the iOS touch workaround described below.
