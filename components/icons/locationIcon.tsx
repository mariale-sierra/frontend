...
/** The Ionicons glyph for each location — also for anything that draws its own icon (a badge). */
export const LOCATION_ICON_NAME: Record<LocationType, keyof typeof Ionicons.glyphMap> = {
  home: 'home-outline',
...
export function LocationIcon({ type, size = 'md', variant = 'circle', color }: LocationIconProps) {
  ...
  return (
    ...
    >
      <Ionicons name={LOCATION_ICON_NAME[type]} size={iconSize[size]} color={colors.paper} />
    </View>
  );
}
